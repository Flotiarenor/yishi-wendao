...
| 探索边界（两层迷雾） | **已接玩法**：`WorldState.explored` = 玩家**真的走过的格**，底图 `style=composed` 把这些格**换成实景风**、别处仍是舆图上淡墨——"我亲自到过的"与"我听说/买来的"一眼可分（定案 §5）。写入路径：`discover_here()`（落地踏勘）+ **`_set_pos_arrived(path_cells=…)` 记整条途经路线**（只记落点会看不见：一格 50 里在 8000 里视野里仅 3.5px）+ 周边 `EXPLORE_RADIUS_CELLS=2` 格。⚠️ **买情报只写 `discovered`、绝不写 `explored`**（听说 ≠ 去过，否则买一份情报就点亮一片实景）；探索依据在**服务端存档**取，前端参数改不动。存档 world 键 **7 个**（`explored` 存格坐标 `[cx, cy]`，非位图） |
