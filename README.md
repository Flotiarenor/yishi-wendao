...
> 然后 `docs/交接-2026-09-12-迷雾三层模型.md`（**最新交接**：现状 + 未实现的迷雾"三层橡皮擦"设计），
> 再 `docs/会话交接.md`（取舍与未决问题）。深读顺序：
...
| 项 | 当前值 |
|---|---|
| 地图缩放 | **已接玩法（T6 缩放）**：档位制 `1000 / 2000 / 4000 / 8000 / 20000` 里（前端 `MAP_SPANS`，存 localStorage；默认 8000 = 原 `MAP_VIEW_RADIUS_LI×2`，**默认视野不变**）。为什么档位而不是连续：底图由服务端按 `span` 渲成并做 LRU 缓存，连续缩放会让缓存几乎不命中。**下发半径随视图走**（`/api/state?span=…` → `map_view(span_li=…)`，半径 = span/2、上限夹到世界边长）——否则缩小后"底图上有城镇、却没有可点击的点"（实测 span 8000→20000 时下发城镇 17 → 161 座） |
| 舆图细节分级 | **按视野跨度分两档**（`tools/atlas.py _detail_tier`）：`far`（span ≥ 16000、一格 ≤ 1.8px）只画 **海 / 山 / 主城 / 官道 / 河流**，主城名 ≤ 26；`near` 给全（沙·草·林·沼·禁制母题 + 小径 + 全部 206 镇）。理由：舆图是逐格扫描，一格占 `px*50/span` 像素，整世界尺度下把 16 万格母题全撒出来只会糊成一片噪声。实测 20000 里/far 画 25/206 镇、26 段路、母题只留山 |
| 测试基线 | `.venv\Scripts\python.exe -X utf8 main.py test` → **846 项全过 / 0 败**（14 个单测文件、逐文件实跑相加）+ 冒烟 20 局 |
| 前端 E2E | `cd frontend ; npm run e2e` → **36 项全过**（地图 6a/6b/6b2/6b3/6c：SVG 世界地图 + 候选真实耗时 + **右键进城** + **陈旧路线清理** + 缩放档位有界与复位） |
...
| 探索边界（两层迷雾） | **当前实现**：`WorldState.explored` = 玩家**真的走过的格**，底图 `style=composed` 把这些格**替换**成实景风、别处仍是舆图上淡墨。写入路径：`discover_here()`（落地踏勘）+ `_set_pos_arrived(path_cells=…)` 记整条途经路线 + 周边 `EXPLORE_RADIUS_CELLS=2` 格。⚠️ **买情报只写 `discovered`、绝不写 `explored`**（听说 ≠ 去过）；探索依据在**服务端存档**取，前端参数改不动。存档 world 键 **7 个**。⚠️ **本实现已知不好看**（白斑 / 格子 / 不连续），**已被用户定案的三层橡皮擦模型取代，但尚未实现**——见 `docs/交接-2026-09-12-迷雾三层模型.md` §四 |
...
| 真浏览器冒烟 | `tools/e2e_web.py`（Selenium + 无头 Edge **152.0.4191.66，与分发的 WebView2 同版本**）→ **20 项全过 / 0 败**（2026-09-11 续复核）；补 jsdom 的两个原理性盲区：**底图是否真画出来**（canvas 读像素断言"非纯色 + 纸底 + 墨 + 水蓝"）、**CJK 是否真能显示**（量渲染宽度）；另量**缩放是否真的换了图**（`<image>` 的 href 里 span 必须变、且换 span 后底图像素必须不同）。基线 **`tests/baselines/map_baseline.json`（入库，跟着代码走）**（sha256 `48763cb0a3392b46` + 像素统计，**改地图就会变红**，`--update-baseline` 重记） |
| 地图交互 | **右键 = 走到这里**（城镇 / 内容点 / 空地**一视同仁**，不必"左键→按钮→选候选"三步）；左键只看详情。目的地候选是**单一状态**（`plan`），位置或缩放档位一变即判过期收起——不再出现"进城后旧路线还挂着、时间也不变" |
| 地图布局 | 地图**铺满容器**（上限 1200px，不再写死 560px）；底图请求的 `px` 由 `ResizeObserver` 量出的**实际显示边长**决定（取整到 64 的倍数）——不这样做大屏上会被放大成糊 |
