...
| 项 | 当前值 |
|---|---|
| 测试基线 | `.venv\Scripts\python.exe -X utf8 main.py test` → **765 项全过 / 0 败**（13 个单测文件 + 冒烟 20 局） |
...
- ⬜ **P4 剩余：T4-B 情报可买卖 → T4-C 档位语义（已拍板"保持现状"）→ T5 城镇设施/场所限制 → T6 前端地图**
  - ⚠️ **T4-B 被 T5 卡住**：坊市只有出生地那一个（`content/sites.py` 的 `SHISHI`），"买图"没法铺开；
    可先做单坊市版
