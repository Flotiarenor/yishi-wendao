...
# 下发的实地踏勘格上限（`map_view()["explored"]`）。
# 为什么要有上限：长局可能走上千格，`state_data()` 每次都要序列化一遍；
# 而前端画实景区域只需要视图**内**的那些格。上限之外的格仍留在存档里（不丢），
# 只是不下发——真要全部下发时改成"按当前视图范围过滤"更合适（等 T6 缩放落地一起做）。
EXPLORED_SEND_MAX = 4000
# 「走过」的格半径（`_record_corridor`）：记落点/途经格时，连同周边 r 格一起记。
#
# 为什么不是 0：一格 50 里、画面上只有几个像素，只记一格在路上几乎看不见
# （实测：走 3 段 = 4 格 = 36 像素 = 0.01%，用户原话"两张图除了位置不一样没有什么区别"）。
#
# 为什么最终取 2（5×5 格 ≈ 250 里见方）而不是 1：**实测视觉效果**——
#   半径 1：走 6 段 → 183 格 → 变化像素 0.62%
#   半径 2：走 6 段 → 331 格 → 1.09%
#   半径 3：走 6 段 → 485 格 → 1.56%
# 再大就不再是"亲自到过"而是"想当然"了；3 与 2 的观感差别也不值那 50% 的膨胀。
# ⚠️ 这是**审美参数**，真要"一眼看出探索范围"，更该做的是"未探索处叠一层纸色蒙版"
# （把对照度拉开），而不是继续放大半径——留待有真实游玩反馈再定。
EXPLORE_RADIUS_CELLS = 2

# ---------- 情报买卖（P4-T4B，定案 §5「情报是资源，可买卖」）----------
# ⚠️ 第一版占位数值，**待用户标定**（与内容点密度一起调）。货单见 `content/intel.py`。
MAP_LEVEL_ORDER = ("none", "coarse", "detailed")   # 档位序（改档只能往上，不能降）
MAP_LEVEL_LABELS = {"none": "无舆图", "coarse": "粗舆图", "detailed": "详图"}
# 买一份「当地情报」揭示的半径（里）= 12 格。
# 为什么是 600 而不是一个域（4000）：实测 1500 里一次就揭出 **78 处**内容点，
# 等于把"探索"直接跳过（"情报是资源"反而被买崩）。600 里约 12 格、几处内容点，
# 一次买 = 一小片已知，够用且不越位。**要与地图缩放一起标定**。
INTEL_REVEAL_RADIUS_LI = 600.0
