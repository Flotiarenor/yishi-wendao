...
> **进度与现状数字**（测试项数 / E2E 项数 / 前端是否已接地图等）见根 `README.md`「当前状态」事实表——**本文不重复维护**。
...
> ⚠️ **现状提醒（2026-09-10）**：本节的"地图是主页面"指的是**布局位置**（主内容区默认显示 `MapScreen.vue`）。
> 但 `MapScreen.vue` **当前仍只是 P3.7 时代的地点卡片列表**（读 `frontend/src/types/sites.ts` 的 `SITES`，
> 不含坐标、不读 `engine/worldmap.py`）——**尚未接世界地图内核**，改 canvas/SVG 是 **T6** 的事。
> `TownScreen.vue` **不存在**（T5/T6 新建）。详见 `docs/README.md` §六、`docs/地图与场所定案.md` §八。
...
所以这条今天不影响任何东西。**等要加音效时**，实测结论（2026-09-11，见 A.6）指明了两条出路：

1. **音频由宿主进程（Python/C# 侧）播**——音频会话就归我们自己的进程，
   音量合成器里显示我们的名字与图标；**不用换壳**。
2. 换掉 WebView2 系壳层——宿主进程自己拥有音频会话。

### A.6 实测记录：音频会话归属到底跟着什么走（2026-09-11）

用真实音频（`Kirara Magic - Aurora (feat. Shion).flac`，ffmpeg 截 40 秒）做了 A/B，
并用 **CoreAudio 会话枚举（`IAudioSessionManager2`）+ 设备峰值表（`IAudioMeterInformation`）**去量，
而不是靠观察界面。三轮结果一致：

| 阶段 | 在响 | 峰值 | 音量合成器里的会话 |
|---|---|---|---|
| A：网页 `<audio>` 播 | WebView2 | 0.34~0.58 | **`msedgewebview2.exe`（state=1 活跃）** |
| B：改由**宿主进程**（我们的 exe）打开音频设备播 | 我们 | 0.68~1.00 | **我们的 exe（state=1 活跃）**；旧的 WebView2 会话降为 **state=0 非活跃但仍然存在** |
| A+B 同时 | 两边 | 追平 | **两条都列出**（用户实际观察到的"出来两个应用"） |

**三条可用结论**：

1. **归属跟着"谁打开音频设备"，与壳技术栈无关**。网页里播就记在 `msedgewebview2` 头上——
   **换成 C#/WinForms+WebView2 壳也一样**（已用同结构的 C# 试验量过）。分叉点不是壳，是"音频在哪播"。
2. **`msedgewebview2` 的会话不会自己消失**：网页音频停掉后它只是转为**非活跃**（state=0）。
   通常它排在自己的应用名（"Microsoft Edge WebView2"）下、Windows 空闲时会收起，
   所以正常不会误导人；**但"同时播两路"会真的出现两个应用条目**——那纯粹是自找的。
3. 因此在**真的要加音效**时，规则定死：

   > **音效/BGM 一律由宿主进程播放；网页侧永不播音频**（也就不用 `AudioContext`）。
   > 这样音量合成器里只有**一条真正活跃的**会话，名字与图标都是我们的。

   ⚠️ 随之而来的接口约定：**前端里的音量滑块不能直接调音量**（播放器不在网页里），
...
> ⚠️ **本节结论已修正（2026-09-11 晚）**：下面原本写"推荐 miniaudio"，那是**基于"只需放一首循环 BGM"**
> 这一前提给的最小依赖方案；用户随后明确要的是**泛用性**（"后面更多东西加入后要能很好覆盖"）。
> 前提一变结论就翻：**音频与表现层统一用 `pygame-ce`**（原生多路音效 / 音量 / 循环，
> 外加几何、字体、无窗口 Surface 渲染——舆图底图就是用它渲的，见 `tools/atlas.py`）。
> `miniaudio` **一行代码都没进项目**；下面的对照表仍保留作"选型依据"与"会话归属"的证据。
> **当前事实**：`.venv` 装的是 **pygame-ce 2.5.8**；`engine/`、`content/` 永不 import 它（只在 `present/` 与 `tools/`）。
...
2. **要"发出去给别人装"时**再考虑 ②/③：那时的驱动力应该是**分发形态**（单文件 exe、安装包、
   不暴露 Python 环境），而不是"音量合成器里显示什么名字"（那条已由 A.6 证明与壳无关）。
3. **音频**按 A.6 的规则：宿主进程播、网页不播——名字问题自然解决，且不换壳。
